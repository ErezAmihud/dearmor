...
TEMP_DIR: Path = (
    Path(os.environ["TEMP_PATH"]).absolute()
...
def test_single_file(py_file, obfuscation_file, temp_dir):
    ...
    generated_by_us = open(
        str((temp_dir / "dump" / (py_file + "c")).resolve()), "rb"
    ).read()
    generated_by_us: CodeType = pyc_to_code(generated_by_us)

    compiled_file: CodeType = compile(
        open(PY_DIR / py_file, "r").read(), "<string>", "exec"
    )
